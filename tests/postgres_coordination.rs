...
fn rollback_drops_database_before_role() {
    ...
    assert!(
        result.is_err(),
        "the final step fails and triggers the rollback"
    );
    ...
    assert!(drop_db.is_some(), "the DB we created must be dropped");
    assert!(drop_role.is_some(), "the role we created must be dropped");
    assert!(
        drop_db < drop_role,
