...
/// Odoo **16**'s lines, taken verbatim off the tree cloned on the VM.
///
/// the fixture that matters for A-V3-28, and the detail is the **absence**: Odoo
/// 16 has no line for `>= '3.13'`. its newest is `>= '3.12'`, pinning gevent
/// 24.2.1 — released before Python 3.13 existed and with no cp313 wheel. so on
/// Fedora, where the venv is built on 3.13, the applicable pin is one written
/// for an earlier interpreter, pip compiles, and the C generated by an old
/// Cython meets a `_PyLong_AsByteArray` that changed signature.
///
/// verbatim, Odoo's own spacing and comments included: a fixture describing
/// another program's file is taken **from** that file, never rewritten from
/// memory (A-R8-1-ter).
const ODOO16_REQUIREMENTS: &str = "\
...
/// on a **covered** Python the facts are given and no cause is asserted.
///
/// the half that makes the check a check, and A-V3-28 moved where the line
/// falls. it used to be "say nothing at all", on the ground that a wrong
/// diagnosis is worse than none (A-R9-1). true of a *conclusion*; not of a
/// **fact** — which interpreter the venv is on, and which lines this Odoo
/// declares, are both true whatever broke the build, and neither appears
/// anywhere in three hundred lines of `gcc`.
#[test]
fn on_a_covered_python_the_facts_are_given_without_a_verdict() {
    let cfg = MockConfig {
        requirements_content: Some(ODOO18_REQUIREMENTS.to_string()),
        run_as_user_fails_on: Some("requirements-gevent".to_string()),
    ...
    assert!(
        err.contains("3.12"),
        "the interpreter is the fact the wall of gcc does not carry: {err}"
    );
    assert!(
        err.contains("gevent==24.2.1"),
        "the declared lines are the other one: {err}"
    );
    // and still no verdict: the cause here may well be something else.
    assert!(
        !err.contains("newer than Python"),
        "on a covered Python nothing may be asserted about the interpreter: {err}"
    );
}

/// and with an unknown Python, nothing is guessed — but the lines still show.
///
/// `None` means "unknown", neither "fine" nor "too new": nothing is concluded
/// from absent information. what does not depend on it — what Odoo declares —
/// is printed anyway, because withholding it helps no one.
#[test]
fn an_unknown_interpreter_does_not_become_a_guess() {
    ...
    assert!(
        ...
        "not knowing is itself worth saying: {err}"
    );
    assert!(
        err.contains("gevent==24.2.1"),
        "the declared lines do not depend on the interpreter and must still show: {err}"
    );
}

/// the case that was silent, reproduced: Odoo 16 on the **tested** Python.
///
/// A-V3-28, and it is not a hypothesis — it is what `fedora-test` did on
/// 2026-08-16. Fedora builds the venv on Python 3.13, which is *equal* to
/// `NEWEST_TESTED_PYTHON`, so the old gate stayed shut; Odoo 16's newest gevent
/// line is `>= '3.12'` and pins 24.2.1, which has no cp313 wheel, so pip
/// compiled and died on `_PyLong_AsByteArray`. The message written for exactly
/// this failure never printed.
#[test]
fn odoo_16_on_the_tested_python_is_no_longer_silent() {
    let cfg = MockConfig {
        requirements_content: Some(ODOO16_REQUIREMENTS.to_string()),
        run_as_user_fails_on: Some("requirements-gevent".to_string()),
        // the newest tested one, NOT newer: that is the whole point.
        python_version: Some((3, 13)),
        ..Default::default()
    };
    let (mock, _log) = MockSystemOps::new(cfg);
    let mut step = InstallPythonRequirements::with_ops(Box::new(mock));
    ...
    assert!(
        err.contains("3.13"),
        "the interpreter the venv was built on is missing: {err}"
    );
    assert!(
        err.contains("gevent==24.2.1"),
        "the reader cannot see that the applicable pin was written for an older Python \
         unless the lines are shown: {err}"
    );
    assert!(
        err.contains("per interpreter version"),
        "without saying the pins are per interpreter, the lines are just noise: {err}"
    );
    assert!(
        err.contains("Building wheel for gevent"),
        "the original error is gone: explaining is not hiding the evidence: {err}"
